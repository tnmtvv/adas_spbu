...
def find_min_z(points, down_sample_coeff):
    ...
    eps = 0.25
    while len(layer) < 2000 // down_sample_coeff:
...
def get_projection(inlier, cropped_outlier):
    ...
    for point in cropped_outlier.points:
        projection = (
            point
            - (np.dot(plane_equation[:-1], point) + plane_equation[-1])
            * plane_equation[:-1]
        )
...
def two_stage_plane_segmentation(true_pcd, down_sample_coeff=1, is_eval=False):
    if not is_eval:
    ...
    min_z_point = find_min_z(points, down_sample_coeff)
    ...
    second_plane_point_indices = np.where(
        np.abs(np.asarray(points).dot(normal_vector) - d) <= 0.4
    )[0]
    ...
    if is_eval:
        ...
        true_pcd.gt_labels = true_pcd.gt_labels[
            list(set(all_gt).difference(set(second_plane_point_indices)))
        ]
        true_pcd.sem_labels = true_pcd.sem_labels[
            list(set(all_sem).difference(set(second_plane_point_indices)))
        ]
    left_bound, right_bound = find_left_right(list(inlier_cloud.points))

    cur_outlier_points = list(np.asarray(cur_pcd.points))
    cropped_outlier_points = list(
        filter(lambda x: left_bound <= x[1] <= right_bound, cur_outlier_points)
    )
    ...
    cropped_outlier_indices = list(
        filter(
            lambda i: tuple(cur_outlier_points[i]) in set_outlier_points,
            cropped_outlier_indices,
        )
    )
...
def plane_segmentation(gt_pcds, down_smple_coeff=1, is_eval=False):
    all_indices = []
    inlier_clouds = []

    for i, _ in enumerate(gt_pcds):
        if is_eval:
            gt_pcds[i].pcd, inlier_cloud, indices = two_stage_plane_segmentation(
                gt_pcds[i], down_smple_coeff, is_eval
            )
        else:
            gt_pcds[i], inlier_cloud, indices = two_stage_plane_segmentation(
                gt_pcds[i], down_smple_coeff, is_eval
            )
