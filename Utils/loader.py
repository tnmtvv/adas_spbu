...
def build_sem_kitti_labeled_pcds(gt_pcds, label_list, map_label_color, get_AoF: Callable = None):
    gt_labeled_pcds = []
    map_sem_color, necessary_labels = read_labels_info()

    for i, cur_pcd in enumerate(gt_pcds):
        ...
        # getting area in front of the car
        if get_AoF:
            indices = get_AoF(cur_pcd)
            cur_pcd = cur_pcd.select_by_index(indices=indices)
            cur_sem_labels = cur_sem_labels[indices]

        # filling semantic colors
