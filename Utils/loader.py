...
from Utils.DatasetFunctions.AUDI_methods import create_open3d_pc, \
    undistort_image
import re
import yaml
...
def read_algos_params(is_eval=False):
    ...
    with open(path, 'r') as stream:
        try:
            ...
            print exc
    dict_algo_params = data['algos_params']
    dict_algo_str_params = data['string_params']
    dict_algo_numeric_params = data['numeric_params']
    return (dict_algo_params, dict_algo_str_params,
            dict_algo_numeric_params)
...
def read_labels_info(is_eval=False):
    ...
    with open(path, 'r') as stream:
        try:
            ...
            print exc
    dict_sem_color = data['color_map']
    necessary_labels = data['necessary_labels']
    return (dict_sem_color, necessary_labels)
...
def build_point_clouds_and_lidars(lidar_list, num_shots,
    ...
    for (i, file) in enumerate(lidar_list):
        ...
        if image_list:
            ...
            semantic_image_front_center_undistorted = \
                undistort_image(semantic_image_front_center,
                                'front_center')
...
def extract_sem_kitti_pcds(bin_list, label_list, get_AoF):
    gt_pcds = []
...
def build_audi_labeled_pcds(gt_pcds):
    ...
    for cur_pcd in gt_pcds:
        ...
        gt_labeled_pcds.append(LabeledPcd(cur_pcd,
                               gt_labels=cur_true_labels,
                               true_colors=colors,
                               sem_labels=cur_true_labels))
...
def extract_sem_kitti_pcds_labeled(
    ...
    (dict_sem_color, necessary_labels) = read_labels_info(is_eval=True)

    if len(bin_list) == len(label_list):
        for (i, bin_file) in enumerate(bin_list):
            cur_pcd = SemanticKitti_methods.read_pcd_from_bin(bin_file)
            (cur_true_labels, cur_sem_labels) = \
                SemanticKitti_methods.read_labels(label_list[i])
            ...
            for sem_label in set(cur_sem_labels):
                ...
                cur_indices = np.where(cur_sem_labels
                        == sem_label)[0].tolist()
                cropped_indices = np.ma.intersect1d(indices,
                        cur_indices)
                cur_sem_colors[cropped_indices] = \
                    dict_sem_color[sem_label]
            if not dict_label_color:

                # cropping label colors for the first time

                dict_label_color[-1] = (0, 0, 0)
                inst_color_lut = np.random.uniform(low=0.0, high=1.0,
                        size=(len(set(cur_true_labels)), 3))
                for (i, label) in enumerate(set(cur_true_labels)):
                    dict_label_color[label] = inst_color_lut[i, :]
                    set_colors.add(tuple(dict_label_color[label]))
                    cur_indices = np.where(cur_true_labels
                            == label)[0].tolist()
                    cropped_indices = np.ma.intersect1d(indices,
                            cur_indices)
                    cur_colors[cropped_indices] = \
                        dict_label_color[label]
            else:

                # adding some cluster

                for (i, label) in enumerate(set(cur_true_labels)):
                    if label not in dict_label_color.keys():
                        dict_label_color[label] = \
                            pcdUtils.align_color(label, set_colors)
                    cur_indices = np.where(cur_true_labels
                            == label)[0].tolist()
                    if cur_indices:
                        cropped_indices = np.ma.intersect1d(indices,
            ...
            gt_labeled_pcds.append(LabeledPcd(cur_pcd,
                                   gt_labels=cur_true_labels[indices],
                                   sem_labels=cur_sem_labels,
                                   sem_colors=cur_sem_colors,
...
def create_data_lists(
    ...
    if audi_mode:
        sub_files = list(filter(lambda x: x.endswith('.png'),
                         sub_files))
    main_files = sorted(main_files, key=lambda x: \
                        int(re.split(separator, x)[0]))
    sub_files = sorted(sub_files, key=lambda x: int(re.split(separator,
                       x)[0]))

    list_main_data = list(map(lambda x: os.path.join(path_to_main_data,
                          x), main_files))
    list_sub_data = list(map(lambda x: os.path.join(path_to_sub_data,
                         x), sub_files))

    if indx_to < 0:
        indx_to = len(list_main_data)
    return (list_main_data[indx_from:indx_to],
            list_sub_data[indx_from:indx_to])
