...
def match_clusters_best_IoU(gt_label: int, pcd: LabeledPcd, true_indices: npt.NDArray, map_association: Dict, external_iou_map: Dict):
    # colors_cloud_raw = np.zeros(np.shape(pcd.pcd.points))
    # colors_cloud_raw[true_indices] = (1, 0, 0)
    # pcd.pcd.colors = o3d.utility.Vector3dVector(colors_cloud_raw)
    ...
    for label in unique_labels:
        ...
        # colors_cloud_raw = np.zeros(np.shape(pcd.pcd.points))
        # colors_cloud_raw[all_raw_label_indices] = (1, 0, 0)
        # pcd.pcd.colors = o3d.utility.Vector3dVector(colors_cloud_raw)
        # o3d.visualization.draw_geometries([pcd.pcd])
...
def evaluate_IoU(pcd: LabeledPcd, map_raw_true: Dict, flatten_indices_of_interest):
    ...
    for label in unique_gt_labels_of_interest:
        ...
        # best_match_indices = np.where(pcd.raw_labels == alg_label)[0]
        # o3d.visualization.draw_geometries([pcd.pcd.select_by_index(gt_label_indices)])
        # o3d.visualization.draw_geometries([pcd.pcd.select_by_index(best_match_indices)])
