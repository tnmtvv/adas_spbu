...
def match_clusters_most_frequent(
    gt_label: int,
    pcd: LabeledPcd,
    indices: npt.NDArray,
    map_association: Dict,
    map_cluster_num_points: Dict,
):
    raw_labels = pcd.raw_labels[indices]
    for label in set(raw_labels):
        num_of_labeled_points = len(np.where(raw_labels == label)[0].tolist())
        if (
            label in map_association
            and map_cluster_num_points[label] < num_of_labeled_points
        ) or label not in map_cluster_num_points:
...
def match_clusters_best_IoU(
    gt_label: int,
    pcd: LabeledPcd,
    true_indices: npt.NDArray,
    map_association: Dict,
    external_iou_map: Dict,
):
    ...
    for label in unique_labels:
        ...
        IoUs[cur_iou] = label
        if (
            label in external_iou_map and external_iou_map[label] < cur_iou
        ) or label not in external_iou_map:
